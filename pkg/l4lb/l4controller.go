...
func (l4c *L4Controller) sync(task utils.TimestampTask) error {
	...
	svc, exists, err := l4c.ctx.Services().GetByKey(task.SvcKey)
	if err != nil {
		return fmt.Errorf("Failed to lookup service for key %s : %w", task.SvcKey, err)
	}
	if !exists || svc == nil {
		...
		klog.V(3).Infof("Ignoring delete of service %s not managed by L4 controller", task.SvcKey)
		return nil
	}
	...
	if l4c.needsDeletion(svc) {
		klog.V(2).Infof("Deleting ILB resources for service %s managed by L4 controller", task.SvcKey)
		result = l4c.processServiceDeletion(task.SvcKey, svc)
		if result == nil {
			return nil
		}
	...
	if wantsILB, _ := annotations.WantsL4ILB(svc); wantsILB {
		klog.V(2).Infof("Ensuring ILB resources for service %s managed by L4 controller", task.SvcKey)
		result = l4c.processServiceCreateOrUpdate(task.SvcKey, svc)
		if result == nil {
