...
func TestProcessCreateOrUpdate(t *testing.T) {
	...
	addNEG(l4c, newSvc)
	task := utils.TimestampTask{EnqueueTime: time.Now(), SvcKey: getKeyForSvc(newSvc, t)}
	err = l4c.sync(task)
	if err != nil {
	...
	updateILBService(l4c, newSvc)
	task.EnqueueTime = time.Now()
	if err = l4c.sync(task); err != nil {
		t.Errorf("Failed to sync deleted service %s, err %v", task.SvcKey, err)
	}
...
func TestProcessUpdateExternalTrafficPolicy(t *testing.T) {
	...
	addNEG(l4c, svc)
	task := utils.TimestampTask{EnqueueTime: time.Now(), SvcKey: getKeyForSvc(svc, t)}
	err := l4c.sync(task)
	if err != nil {
	...
	task.EnqueueTime = time.Now()
	if err = l4c.sync(task); err != nil {
		t.Errorf("Failed to sync deleted service %s, err %v", task, err)
	}
	...
	for _, onlyLocal := range []bool{true, false} {
		t.Run(fmt.Sprintf("L4 with LocalMode=%v", onlyLocal), func(t *testing.T) {
			...
			for port := 8000; port < 8020; port++ {
				...
				addNEG(l4c, newSvc)
				l4c.svcQueue.Enqueue(utils.TimestampTask{EnqueueTime: time.Now(), SvcKey: getKeyForSvc(newSvc, t)})
			}
