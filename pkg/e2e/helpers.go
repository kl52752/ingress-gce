...
// CheckSvcUpdateEvents checks that the service has 2 consecutive events update and ensure.
func CheckSvcUpdateEvents(s *Sandbox, svcName, updateMsg, ensureMsg string) (bool, error) {
	svc, err := s.f.Clientset.CoreV1().Services(s.Namespace).Get(context.TODO(), svcName, metav1.GetOptions{})
	if svc == nil || err != nil {
		return false, fmt.Errorf("failed to get service %s/%s: %v", s.Namespace, svcName, err)
	}
	...
	var updateEvent *v1.Event
	for _, event := range eventList.Items {
		if event.Type == v1.EventTypeNormal {
			klog.Errorf("Found event of type %v: %v timestamp: %v", event.Type, event.Message, event.LastTimestamp)
			if updateEvent == nil && strings.Contains(event.Message, updateMsg) {
				klog.Errorf("Is Update event %s checked: %s time: %v", updateEvent, event.Message, event.LastTimestamp)
				event := event
				updateEvent = &event
				break
			}
	...
	for _, event := range eventList.Items {
		if event.Type == v1.EventTypeNormal {
			// Check that ensure event happend after update.
			if strings.Contains(event.Message, ensureMsg) && event.LastTimestamp.After(updateEvent.LastTimestamp.Time) {
				return true, nil
			}
		}
	}
...
func WaitForSvcUpdateEvents(s *Sandbox, svcName, upateMsg, ensureMsg string) error {
	return wait.Poll(updateIngressPollInterval, negGCPollTimeout, func() (bool, error) {
		isPresent, err := CheckSvcUpdateEvents(s, svcName, upateMsg, ensureMsg)
		if err != nil {
			klog.Infof("Error waiting for svc event err: %v", err)
		}
