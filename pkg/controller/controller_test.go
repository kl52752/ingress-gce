...
func TestIngressSyncError(t *testing.T) {
	...
	err := lbc.sync(utils.TimestampTask{EnqueueTime: time.Now(), SvcKey: ingStoreKey})
	if err == nil {
	...
	for _, tc := range testCases {
		t.Run(tc.desc, func(t *testing.T) {
			...
			for _, name := range tc.ingNames {
				...
				ingStoreKey := getKey(ing, t)
				if err := lbc.sync(utils.TimestampTask{EnqueueTime: time.Now(), SvcKey: ingStoreKey}); err != nil {
					t.Fatalf("lbc.sync(%v) = %v, want nil", ingStoreKey, err)
				}
...
func TestIngressesWithSharedResourcesWithFinalizer(t *testing.T) {
	...
	if err1 := lbc.sync(utils.TimestampTask{EnqueueTime: time.Now(), SvcKey: ingStoreKey}); err1 != nil {
		t.Fatalf("lbc.sync(%v) = %v, want nil", ingStoreKey, err1)
	}
	if err2 := lbc.sync(utils.TimestampTask{EnqueueTime: time.Now(), SvcKey: otherIngStoreKey}); err2 != nil {
...
func TestMCIngressIG(t *testing.T) {
	...
	if err2 := lbc.sync(utils.TimestampTask{EnqueueTime: time.Now(), SvcKey: mcIngStoreKey}); err2 != nil {
	...
	if err := lbc.sync(utils.TimestampTask{EnqueueTime: time.Now(), SvcKey: mcIngStoreKey}); err != nil {
